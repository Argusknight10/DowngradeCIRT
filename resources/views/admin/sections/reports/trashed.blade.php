<x-mainAdmin>
  <x-slot:title>Trash Reports</x-slot:title>

  <div class="p-6 sm:ml-64 my-20">
    <h1 class="text-4xl text-[#14477A] mb-4">Sampah Aduan</h1>
    {{-- toas --}}
    @if (session('success'))
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td class="px-6 py-4">
                @if ($report->is_solved == 0)
                  <span class="flex justify-center items-center text-sm font-medium text-gray-900 dark:text-white"><span
                      class="flex w-2.5 h-2.5 bg-red-500 rounded-full mr-1.5 flex-shrink-0"></span>Belum ditangani</span>
                @else
                  <span class="flex justify-center items-center text-sm font-medium text-gray-900 dark:text-white"><span
              ...
              <td class="px-6 py-4 flex justify-center items-start gap-2">
                <form action="{{ route('admin.trashed.reports.restore', $report->id) }}" method="post">
                  @csrf
                  <button type="submit" onclick="return confirm('Anda yakin ingin mengembalikan laporan ini?')">
                ...
                <form action="{{ route('admin.trashed.reports.force', $report->id) }}" method="post">
                  @csrf
                  @method('delete')
                  <button type="submit" onclick="return confirm('Anda yakin ingin menghapus permanen laporan ini?')">
